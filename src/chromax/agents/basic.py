...
- Always cite specific files (e.g. `src/auth.py`) when referring to code.
- If you fetch a file, reference line numbers or sections when relevant.
- If you cannot find information, say so clearly — do not guess or hallucinate file names.
- Be concise: lead with the direct answer, then provide supporting evidence.
"""


# ---------- LangChain tools (wrapping the raw functions) ----------
...
TOOLS = [tool_get_readme, tool_get_repo_structure, tool_get_file_content]
...
def _make_llm() -> ChatGroq:
    # Auto-reads GROQ_API_KEY from env
